...
man_n = 5           #количество человек
yoyo_n = n - man_n  #количество йо-йо
dimen = 5           #мерность пространства
c_min = -5.12        #минимум пространства поиска
c_max = 5.12           #максимум пространства поиска
t_end = 1          #время окончания программы
sm_yoyo_n = 10       #количество маленьких йо-йо
lg_threshold = 0.05
step_size = 0.5
size_step = 0.01
sm_alpha = 0.001
...
def fit_func(x):
    return 10*dimen + sum([((x[i])**2 - 10 * m.cos(2 * m.pi * x[i])) for i in range(dimen)])
...
for var_num in range(10):
    ...
    for dimen in dimens:
        ...
        for t_end in times:
            ...
            plt.xlabel('Итерации')
            graph_name = '_'.join(['rastrigin',str(var_num+1),str(dimen),str(t_end),'.png'])
            graph_x = []
            graph_y = []
            ...
            while  (t.time() - t_start) < t_end * 60:
                ...
                for i in range(man_n):                                      #проход по мужчинам
                    for j in man[i][2]:                                    #проход по йо-йо выбранного мужика
                        while dist_check(man[i][0], lg_yoyo[j][0], lg_threshold) == 0:    #проверка расстояния между мужиком и йо-йо
                ...
                for i in range(bl_k):
                    ...
                    for j in range(dimen):
                        if j == bl_selected_dim[i]:
                            bl_step[i][0].append(man[0][0][j]+(man[0][0][j]*r.uniform(-1,1)*bl_gam)**r.randint(1,bl_lamb))
                        else:
